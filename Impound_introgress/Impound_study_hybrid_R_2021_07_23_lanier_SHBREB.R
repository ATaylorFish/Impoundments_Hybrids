...
x <- read.structure("Lanier_inputs_for_R_Introgress3.stru", n.ind = 177, n.loc = 16, onerowperind = TRUE,
...
hi.index
write.csv(hi.index, "SHBxREB_hi_index.csv")
...
in.het
write.csv(in.het, "SHBxREB_in_het.csv")
...
triPlot <- triangle.plot(hi.index=hi.index, int.het=in.het, pdf=TRUE, out.file="SHBxREB_tri.pdf")
...
mk.image(introgress.data=introgress_input, loci.data=loci.data, 
         ...
         xlab.h="REB ancestry", pdf=TRUE, out.file="SHBxREB_image.pdf")
...
clines.plot(cline.data=cline.out, rplots=3, cplots=3, pdf=TRUE, out.file="SHBxREB_clines.pdf")

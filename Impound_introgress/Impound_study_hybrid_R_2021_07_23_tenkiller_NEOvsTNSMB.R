...
x <- read.structure("Tenkiller_inputs_for_R_Introgress.stru", n.ind = 308, n.loc = 7, onerowperind = TRUE,
...
loci.data<- read.csv(file="locidata_tenkiller.txt", header=TRUE)
...
p1.data <- filter(xdf, pop=="refNEO")
p1.data <- select(p1.data, -c(ind, pop))
p1.data <- t(p1.data)
p2.data <- filter(xdf, pop=="refTNSMB")
...
hi.index
write.csv(hi.index, "NEOxTNSMB_hi_index.csv")
...
in.het
write.csv(in.het, "NEOxTNSMB_in_het.csv")
...
triPlot <- triangle.plot(hi.index=hi.index, int.het=in.het, pdf=TRUE, out.file="NEOxTNSMB_tri.pdf")
...
mk.image(introgress.data=introgress_input, loci.data=loci.data, 
         ...
         xlab.h="TNSMB ancestry", pdf=TRUE, out.file="NEOxTNSMB_image.pdf")
...
clines.plot(cline.data=cline.out, rplots=3, cplots=3, pdf=TRUE, out.file="NEOxTNSMB_clines.pdf")

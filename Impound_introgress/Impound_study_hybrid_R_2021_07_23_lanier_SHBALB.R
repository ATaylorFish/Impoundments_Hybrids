...
x <- read.structure("Lanier_inputs_for_R_Introgress1.stru", n.ind = 328, n.loc = 16, onerowperind = TRUE,
...
hi.index
write.csv(hi.index, "SHBxALB_hi_index.csv")
...
in.het
write.csv(in.het, "SHBxALB_in_het.csv")
...
triPlot <- triangle.plot(hi.index=hi.index, int.het=in.het, pdf=TRUE, out.file="SHBxALB_tri.pdf")
...
mk.image(introgress.data=introgress_input, loci.data=loci.data, 
         ...
         xlab.h="ALB ancestry", pdf=TRUE, out.file="SHBxALB_image.pdf")
...
clines.plot(cline.data=cline.out, rplots=3, cplots=3, pdf=TRUE, out.file="SHBxALB_clines.pdf")


####hybriddetective analysis to assign individuals to hybrid classes, 
#### and evaluate power of msat loci to diagnose the classes
